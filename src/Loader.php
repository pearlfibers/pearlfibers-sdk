<?php
/**
 * The main loader class for pearlfibers SDK
 *
...
use pearlfibersSDK\Common\Module_Factory;
...
/**
 * Singleton loader for pearlfibers SDK.
 */
final class Loader {
	/**
	 * Singleton instance.
	...
	public static function init() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Loader ) ) {
			...
			$modules        = array_merge( self::$available_modules, apply_filters( 'pearlfibers_sdk_modules', [] ) );
			foreach ( $modules as $key => $module ) {
				if ( ! class_exists( 'pearlfibersSDK\\Modules\\' . ucwords( $module, '_' ) ) ) {
					unset( $modules[ $key ] );
				}
			}
	...
	public static function get_cache_token() {
		$cache_token = get_transient( 'pearlfibers_sdk_cache_token' );
	...
	public static function clear_cache_token() {
		delete_transient( 'pearlfibers_sdk_cache_token' );
	}
