...
/**
 * Licenser module for pearlfibers SDK.
 */
class Licenser extends Abstract_Module {
	...
	public function license_view() {
		...
		?>
		<style type="text/css">
			input.pearlfibers-sdk-text-input-valid {
				border: 1px solid #7ad03a;
			}

			input.pearlfibers-sdk-license-input {
				width: 300px;
				padding: 0 8px;
				line-height: 2;
				min-height: 30px;
			...
			.pearlfibers-sdk-license-deactivate-cta {
				color: #fff;
				background: #7ad03a;
			...
			.pearlfibers-sdk-license-activate-cta {
				color: #fff;
				background: #dd3d36;
				display: inline-block;
				text-decoration: none;
			...
			button.button.pearlfibers-sdk-licenser-button-cta {
				line-height: 26px;
				height: 29px;
				vertical-align: top;
			}
		...
		<?php
		echo sprintf(
			'<p>%s<input class="pearlfibers-sdk-license-input %s" type="text" id="%s_license" name="%s_license" value="%s" /><a class="%s">%s</a>&nbsp;&nbsp;&nbsp;<button name="%s_btn_trigger" class="button button-primary pearlfibers-sdk-licenser-button-cta" value="yes" type="submit" >%s</button></p><p class="description">%s</p>%s',
			( ( 'valid' === $status ) ? sprintf( '<input type="hidden" value="%s" name="%s_license" />', esc_attr( $value ), esc_attr( $this->product->get_key() ) ) : '' ),
			( ( 'valid' === $status ) ? 'pearlfibers-sdk-text-input-valid' : '' ),
			...
			esc_attr( ( 'valid' === $status ? 'pearlfibers-sdk-license-deactivate-cta' : 'pearlfibers-sdk-license-activate-cta' ) ),
	...
	public function do_license_process( $license, $action = 'toggle' ) {
		if ( strlen( $license ) < 10 ) {
			return new \WP_Error( 'pearlfibers-license-invalid-format', 'Invalid license.' );
		}
		$status = $this->get_license_status();

		if ( 'valid' === $status && 'activate' === $action ) {
			return new \WP_Error( 'pearlfibers-license-already-active', 'License is already active.' );
		}
		if ( 'valid' !== $status && 'deactivate' === $action ) {
			return new \WP_Error( 'pearlfibers-license-already-deactivate', 'License not active.' );
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error( 'pearlfibers-license-500', sprintf( 'ERROR: Failed to connect to the license service. Please try again later. Reason: %s', $response->get_error_message() ) );
		}

		$license_data = json_decode( wp_remote_retrieve_body( $response ) );

		if ( ! is_object( $license_data ) ) {
			return new \WP_Error( 'pearlfibers-license-404', 'ERROR: Failed to validate license. Please try again in one minute.' );
		}
		if ( 'check' === $action ) {
			return $license_data;
		}
		...
		if ( 'activate' === $action && 'valid' !== $license_data->license ) {
			return new \WP_Error( 'pearlfibers-license-invalid', 'ERROR: Invalid license provided.' );
		}
	...
	public function update_nag() {
		...
		$update_message = apply_filters( 'pearlfibers_sdk_license_update_message', 'Updating this theme will lose any customizations you have made. Cancel to stop, OK to update.' );
	...
	public function load( $product ) {
		...
		if ( false !== $namespace ) {
			...
			add_filter( 'pearlfibers_sdk_license_process_' . $namespace, [ $this, 'do_license_process' ], 10, 2 );
