<?php
/**
 * The notification model class for pearlfibers SDK
 *
...
/**
 * Notification module for pearlfibers SDK.
 */
class Notification extends Abstract_Module {
	...
	private static function set_last_active_notification_timestamp() {
		...
		update_option( 'pearlfibers_sdk_notifications', $metadata );
	}
	...
	public static function get_notification_html( $notification_details ) {
		...
		$notification_html = '<div class="notice notice-success is-dismissible pearlfibers-sdk-notice" data-notification-id="' . esc_attr( $notification_details['id'] ) . '" id="' . esc_attr( $notification_details['id'] ) . '-notification"> <div class="pearlfibers-sdk-notification-box">';
	...
	public static function render_snippets() {
		...
		?>
		<style type="text/css">
			.pearlfibers-sdk-notification-box {
				padding: 3px;
			}

			.pearlfibers-sdk-notification-box .actions {
				margin-top: 6px;
				margin-bottom: 4px;
			}

			.pearlfibers-sdk-notification-box .button {
				margin-right: 5px;
			}
		</style>
		<script type="text/javascript">
			(function ($) {
				$(document).ready(function () {
					$('#wpbody-content').on('click', ".pearlfibers-sdk-notice a.button, .pearlfibers-sdk-notice .notice-dismiss", function (e) {

						var container = $('.pearlfibers-sdk-notice');
						var link = $(this);
						...
						$.post(
							ajaxurl,
							{
								...
								'action': 'pearlfibers_sdk_dismiss_notice',
								'id': notification_id,
								'confirm': confirm
							}
						);
						if (confirm === 'yes') {
							$(this).trigger('pearlfibers-sdk:confirmed');
						} else {
							$(this).trigger('pearlfibers-sdk:canceled');
						}
						container.hide();
						if (link.attr('href') === '#') {
							return false;
	...
	public static function setup_notifications() {
		$notifications       = apply_filters( 'pearlfibers_sdk_registered_notifications', [] );
		$notifications       = array_filter(
			$notifications,
			function ( $value ) {
				if ( ! isset( $value['id'] ) ) {
	...
	public function load( $product ) {
		if ( apply_filters( 'pearlfibers_sdk_hide_notifications', false ) ) {
			return;
		}
		...
		add_action( 'wp_ajax_pearlfibers_sdk_dismiss_notice', array( __CLASS__, 'dismiss' ) );
