<?php
/**
 * The deactivate feedback model class for pearlfibers SDK
 *
...
/**
 * Uninstall feedback module for pearlfibers SDK.
 */
class Uninstall_Feedback extends Abstract_Module {
	 ...
	 */
	const FEEDBACK_ENDPOINT = 'https://api.pearlfibers.com/tracking/uninstall';
	...
	private function render_theme_feedback_popup() {
		...
		$info_disclosure_link = '<a href="#" class="info-disclosure-link">' . apply_filters( $this->product->get_slug() . '_pearlfibers_sdk_info_collect_cta', 'What info do we collect?' ) . '</a>';
	...
	private function get_disclosure_labels() {
		$disclosure_new_labels = apply_filters( $this->product->get_slug() . '_pearlfibers_sdk_disclosure_content_labels', [], $this->product );
		$disclosure_labels     = array_merge(
			[
				'title' => 'Below is a detailed view of all data that pearlfibers will receive if you fill in this survey. No domain name, email address or IP addresses are transmited after you submit the survey.',
				'items' => [
