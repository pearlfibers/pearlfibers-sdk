<?php
/**
 * The blog dashboard model class for pearlfibers SDK
 *
...
namespace pearlfibersSDK\Modules;

use pearlfibersSDK\Common\Abstract_Module;
use pearlfibersSDK\Product;
...
/**
 * Blog dashboard widget module for pearlfibers SDK.
 */
class Dashboard_Widget extends Abstract_Module {
	...
	public function load( $product ) {
		if ( apply_filters( 'pearlfibers_sdk_hide_dashboard_widget', false ) ) {
			return;
		}
		$this->product        = $product;
		$this->dashboard_name = apply_filters( 'pearlfibers_sdk_dashboard_widget_name', 'WordPress Guides/Tutorials' );
		$this->feeds          = apply_filters(
			'pearlfibers_sdk_dashboard_widget_feeds',
			[
				'https://pearlfibers.com/blog/feed',
			]
		);
		add_action( 'wp_dashboard_setup', array( &$this, 'add_widget' ) );
		add_action( 'wp_network_dashboard_setup', array( &$this, 'add_widget' ) );
		add_filter( 'pearlfibers_sdk_recommend_plugin_or_theme', array( &$this, 'recommend_plugin_or_theme' ) );
	...
	public function add_widget() {
		...
		if ( isset( $wp_meta_boxes['dashboard']['normal']['core']['pearlfibers'] ) ) {
			return;
		}
	...
	public function render_dashboard_widget() {
		...
		?>
		<style type="text/css">
			#pearlfibers ul li.ti-dw-recommend-item {
				padding-left: 7px;
		...
		<?php do_action( 'pearlfibers_sdk_dashboard_widget_before', $this->product ); ?>
		...
		<?php
		$recommend = apply_filters( 'pearlfibers_sdk_recommend_plugin_or_theme', array() );
		...
		<div class="ti-dw-footer">
					<span class="ti-dw-recommend-item ">
							<span class="ti-dw-recommend"><?php echo esc_attr( apply_filters( 'pearlfibers_sdk_dashboard_popular_label', sprintf( 'Popular %s', ucwords( $type ) ) ) ); ?>
								: </span>
						<?php
						echo esc_attr(
							trim(
						...
						(<a class="thickbox open-plugin-details-modal"
							href="<?php echo esc_url( $url . '&TB_iframe=true&width=600&height=500' ); ?>"><?php echo esc_attr( apply_filters( 'pearlfibers_sdk_dashboard_install_label', 'Install' ) ); ?></a>)
					</span>
			<span class="ti-dw-powered-by"><span><?php echo esc_attr( apply_filters( 'pearlfibers_sdk_dashboard_widget_powered_by', sprintf( 'Powered by %s', $this->product->get_friendly_name() ) ) ); ?></span></span>
		</div>
	...
	private function setup_feeds() {
		if ( false === ( $items_normalized = get_transient( 'pearlfibers_sdk_feed_items' ) ) ) { //phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure
			...
			set_transient( 'pearlfibers_sdk_feed_items', $items_normalized, 48 * HOUR_IN_SECONDS );
		}
	...
	public function get_product_from_api() {
		if ( false === ( $products = get_transient( 'pearlfibers_sdk_products' ) ) ) { //phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure
			$products                = array();
			$all_themes              = $this->get_themes_from_wporg( 'pearlfibers' );
			$all_plugins             = $this->get_plugins_from_wporg( 'pearlfibers' );
			...
			set_transient( 'pearlfibers_sdk_products', $products, 6 * HOUR_IN_SECONDS );
		}
